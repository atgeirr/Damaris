...
# DD (AKA Damaris Data) is a dictionary that has been filled by the Damaris server process with NumPy arrays
# that point to the data variables that is exposed in the simulation. The Damaris source file that implements this
# is PyAction, found in the src/scripts/ and include/damaris/scripts directories. Damaris <variables> must be 
# exposed to the Python <pyscript> XML element by including its name i.e. MyPyAction in the following example
# 
...
# DD['block_domains'] - A single simulation iteration might write multiple blocks of data (if domains in the simulation XML file  > 1)
#
# N.B. Only the data for the current iteration is available - and it is Read Only. If it is needed later it needs to be saved 
#      (pickleed maybe) and re-read on the next iteration. When connected to a Dask scheduler then the data can be saved on 
#      the distributed workers.
#      Also, this file is read by each Damaris server process on each iteration that is specified by the frequency="" XML sttribute.
def main(DD):
