...
{
    for(int j=0; j <= sim->life.Ncolumns; j++)
        sim->life.rmesh_x[j] = sim->par_rank + 1; // sim->cycle;


    for(int i=0; i <= sim->life.Nrows; i++)
        sim->life.rmesh_y[i] = sim->par_rank + 10; // sim->cycle;
...
void exposeDataToDamaris(simulation_data* sim) {
    ...
    int64_t position_life[2] = {0 , sim->par_rank*sim->life.Nrows};


	if(firstCall == 0) {
        int64_t position_y2d[1] = {sim->par_rank * sim->life.Nrows};

        printf("Position for y2d is: %d -- rank is: %d\n" , position_y2d[0] , sim->par_rank);

        damaris_set_position("coordinates/y2d",position_y2d);
        //damaris_set_position("coordinates/x2d",position_y2d);
...
int main(int argc, char **argv)
{
	...
	damaris_initialize("life-hdf5.xml",all);
	
	damaris_parameter_get("NROW",&NROW,sizeof(NROW));
	damaris_parameter_get("NCOL",&NCOL,sizeof(NCOL));
	...
	if((err == DAMARIS_OK || err == DAMARIS_NO_SERVER) && is_client) {
		...
		{
			fprintf(stderr,"The total number of rows does not divide");
			fprintf(stderr," evenly by the number of MPI tasks.\n");
			exit(1);
		}
