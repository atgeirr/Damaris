...
#         Test the base image that (contains no extra dependncies):
#          > sudo ./test_damaris_images.sh  damaris-devlopment  "-"  "cmake --version"
#         Get a bash prompt in base image:
#         (change  DOCKERFILE_ARRAY to select version to run):
...
do
    BASE_IMAGE_SHORT=$(get_tag_name "../bases/Dockerfile.${DOCKERFILE}.nodeps")
...
echo "GRAPH: Docker image base : $DOCKER_IMAGE_BASENAME<O.S.>-nodeps-$DAMARIS_VER  "
...
# Add a column for the single no-dependencies table column
  TABLE_HEAD+="| no-deps "
  TABLE_BASE+="|--------"
done
echo "$TABLE_HEAD|"
...
do
    ...
    if [[ "$BASE_IMAGE_SHORT" != "" ]] ; then        
          ...
          BASEIMAGETAG=$(echo $BASE_IMAGE_SHORT-nodeps${DAMARIS_VER})
