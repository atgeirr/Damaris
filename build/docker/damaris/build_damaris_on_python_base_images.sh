...
# Date: 12/4/2022
# Usage:   ./build_damaris_on_python_base_images.sh  DAMARIS_REPO  DAMARIS_VER   MY_CI_READ_REPO_PWD
...
#          the FROM part of the file Dockerfile.${DOCKERFILE}.python .
#
# Requirements: 
#         A set of files in directory:  ../bases/Dockerfile.${DOCKERFILE}.python
#         A set of built base images in registry.gitlab.inria.fr/damaris/damaris-development that
#         correspond to the Dockerfiles (Dockerfile.${DOCKERFILE}.python)
#         A file: Docker._BASEWITHPARAVIEW_.python that has template strings to replace using sed:
#           _BASEWITHPARAVIEW_  _INSTALL_GFORT_  _PYTHON_ON_OR_OFF_
#
# Outputs: 
...
#        where BASEIMAGETAG == $(echo $BASE_IMAGE_SHORT-python)
#        and BASE_IMAGE_SHORT is derived from the FROM tag in the base Dockerfiles 
#        (i.e. derived from  ../bases/Dockerfile.${DOCKERFILE}.python using function get_tag_name() below)
#       /bin/bash -c
...
do
    ...
    # Get variables for replacing in Dockerfile._BASEWITHPARAVIEW_.python templated file
...
echo "GRAPH: pf : python base not built  "
echo "GRAPH:   "
