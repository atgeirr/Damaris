...
(reps. \function{df\_end\_iteration}) will inform the dedicated cores that the iteration $i$ has finished.
This will update potential backends such as VisIt, and give more information to \Damaris{} about
the average durations of an iteration.
...
\begin{tabular}{|l|}
   ...
   \hline
   \hline
   \function{int DC\_initialize(char* configfile, int rank)}  \\
   \function{int DC\_end\_iteration(int iteration)}  \\
   ...
   \hline
   \hline
   \function{df\_initialize(character* configfile, integer rank, integer ierr)} \\
   \function{df\_end\_iteration(integer iteration, integer ierr)} \\
...
made and all the threads one a node should act as one single client (the XML configuration file
...
to start one server per node even if process ranks are not consecutive in the node.
...
\subsubsection{Writing chunks of variables}

It may be necessary to write chunks of variables instead of full variables. 
For well-dimensioned layout, a chunk
must have the same numbers of dimensions than the variable's layout, and its boundaries
must fall within the extents of the layout.
Listings~\ref{writingChunkC} and~\ref{writingChunkF90} show how to define a chunk handle,
which packs together the number of dimensions and the boundaries.
In the example, the chunk extent along the first dimension is $4 - 2 + 1 = 3$ (the last index is included
in the chunk).
A chunk handle is not attached to any particular variable and can be used in several calls 
to \function{DC\_chunk\_write}.

Chunks are also the only mean to write on a variable that have an unlimited layout, in which case
the dimensionality of the chunk must be 1.

\noindent\begin{minipage}{\textwidth}
\vspace{0.5cm}
\lstset{language=C,caption={Writing a chunk of variable to \Damaris{} in C},label=writingChunkC}
\lstinputlisting[language=C]{listings/chunk.c}
\end{minipage}

\noindent\begin{minipage}{\textwidth}
%\vspace{0.5cm}
\lstset{language=Fortran,caption={Writing a chunk of variable to \Damaris{} in Fortran},label=writingChunkF90}
\lstinputlisting[language=Fortran]{listings/chunk.f90}
\end{minipage}

\subsubsection{Direct access to the shared memory}

The API presented above has the disadvantage to copy local data into the shared memory.
...
Equivalent functions to allocate chunks have not been yet implemented in \Damaris{}.
...
This method is currently only available in C, and allows only a ``core'' scope. Besides, the event
\emph{should not} be already defined in the configuration file.
