...
bool PointMesh::ExposeVisItMetaData(visit_handle md) 
{
	...
	if(VisIt_MeshMetaData_alloc(&m1) == VISIT_OKAY)
	{
		...
		int nbrLocalBlocksPerClient = 
			Environment::NumDomainsPerClient();
...
bool PointMesh::ExposeVisItData(visit_handle* h, 
	int source, int iteration, int block)
{
	...
	if(VisIt_PointMesh_alloc(h) != VISIT_ERROR) {
		...
		vx = GetCoord(0); //VariableManager::Search(it->name());
		if(not vx) {
		...
		if(vy->GetLayout()->GetDimensions() != 1) {
			CFGERROR("Wrong number of dimensions "
				<< "for coordinate " << vy->GetName());
			return false;
		}
		//it++;
		...
		if(GetNumCoord() == 3) {
		vz = GetCoord(2); //VariableManager::Search(it->name());
		if(not vz) {
...
std::shared_ptr<vtkDataSet> PointMesh::GetVtkGrid(int source , int iteration , int block)
{
