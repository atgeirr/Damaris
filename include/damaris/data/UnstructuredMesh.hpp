...
private:
    ...
    /**
     * Is the total number of client ranks that this server is looking after.
     * It used to set the size of std::vector<vtkUnstructuredGrid* > vect_vtk_usm_grid_.
     */
    int source_total_ ;
...
public:
        ...
        */
       void SetSourceRange(int source_range_low, int source_range_high) {
           if (vect_vtk_usm_grid_.size() == 0) {
                ...
                // We assume there is only one block per source for the UnstructuredMesh data Variable sect_vtk_sizes.
        ...
        int GetTotalMeshSections(int iteration) {
            ...
            // Get the block (that corresponds to the block of the enclosing field Variable)
            sectn_block =  sect_sizes_var->GetBlock(source_range_low , iteration , block) ;
            // Get the number of sections of the mesh held by this block
            // - there is 1 array element per mesh section.
            n_sections_total = sectn_block->GetGlobalExtent(dimension);
            return n_sections_total ;
        }

        /**
		 *  Return the sum of sections in the mesh up to the input source rank.
		 *   - This is done by looking at the lower_bound_ value of the block of mesh variable
		 ...
		 *
		 */
        int GetGlobalOffsetOfSections(int source)  {
        	std::shared_ptr<Variable> sect_sizes_var;
			std::shared_ptr<Block> sectn_block;
			int global_section_offset ;
			...
			// Get the number of sections of the mesh held by this block
			// - there is 1 array element per mesh section.
			global_section_offset = sectn_block->GetStartIndex(dimension);
